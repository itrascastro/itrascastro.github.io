...
class GlobalViewRenderer extends CalendarRenderer {
    ...
    calculateDominantColor(events, calendar) {
        ...
        if (systemEvents.length > 0) {
            ...
            return categoryService.getCategoryColor(systemEvents[0].categoryId, calendar);
        }
        
        // PRIORITAT 2: Si només hi ha un esdeveniment (i no és de sistema)
        if (events.length === 1) {
            return categoryService.getCategoryColor(events[0].categoryId, calendar);
        }
        ...
        // En cas d'empat, agafar la categoria amb ID més petit (ordre alfabètic)
        const dominantCategoryId = dominantCategories.sort()[0];
        
        return categoryService.getCategoryColor(dominantCategoryId, calendar);
    }
