...
class ViewManager {
    ...
    renderCompactView(calendar) {
        ...
        if (container && !container.querySelector('#compactZoomSlider')) {
            ...
            slider.innerHTML = `
                <div class="compact-zoom-controls">
                    <input id="compactZoomRange" type="range" min="60" max="160" step="5" value="${Math.round(z*100)}" aria-label="Zoom vista compacta">
                    <button type="button" class="compact-fullscreen-btn" data-action="open-compact-fullscreen" title="Mostrar vista compacta ampliada" aria-label="Mostrar vista compacta ampliada">
                        <svg width="16" height="16" viewBox="0 0 16 16" role="presentation" focusable="false" aria-hidden="true">
                            <path d="M1 5V1h4M11 1h4v4M5 15H1v-4M15 11v4h-4" stroke="currentColor" stroke-width="1.5" fill="none" stroke-linecap="round" stroke-linejoin="round"></path>
                        </svg>
                    </button>
                </div>
            `;
