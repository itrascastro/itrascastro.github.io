...
document.addEventListener('DOMContentLoaded', function () {
  ...
  document.querySelectorAll('.checklist .checklist-item, .checklist-list .checklist-item').forEach(item => {
...
function initializeBookmark(){
  ...
  function getBookmark(){
    ...
    try {
      const bm = JSON.parse(localStorage.getItem('quadern_bookmark') || 'null');
      if (bm) {
        setBookmark(bm);
  ...
  function getPageContext(){
    ...
    const base = document.body.getAttribute('data-baseurl')||''; const path = location.pathname||''; const pageUrl = path.startsWith(base) ? path : base + path;
  ...
  btn.addEventListener('click', function(e){
    ...
    const baseurl = document.body.getAttribute('data-baseurl')||''; const path = location.pathname||''; const hereUrl = path.startsWith(baseurl) ? path : baseurl + path;
